...
DriverlessEstimatorRunner make_runner()
{
    ...
    auto transform_buffer = std::make_shared<transforms::TransformBuffer>(1000000);
    transform_buffer->set_base_to_gss(transforms::RigidTransform2D{1.0, 0.25, 0.0});

    return DriverlessEstimatorRunner{
        std::make_shared<LatestEstimate>(),
        transform_buffer,
        ekf_params,
        gss_config,
        false};
...
ImuMeasurement make_imu(std::uint64_t timestamp_ns)
{
...
GssMeasurement make_gss(std::uint64_t timestamp_ns)
{
    GssMeasurement measurement;
    measurement.timestamp_ns = timestamp_ns;
...
TEST(DriverlessEstimatorRunnerTest, MultipleProducerThreadsAreSafe)
{
    ...
    runner.start();
    EXPECT_TRUE(wait_until_stats(runner, kProducerCount * kEventsPerProducer, 0));
    runner.stop();
