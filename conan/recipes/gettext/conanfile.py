...
from conan.tools.files import patch, copy, download, get, rmdir
...
class GetTextConan(ConanFile):
    ...
    def source(self):
        ...
        for entry in self.conan_data.get("patches", {}).get(self.version, []):
            download(self, url=entry["url"],
                     filename=os.path.join(self.source_folder, entry["patch_file"]),
                     sha256=entry["sha256"])
    ...
    def build(self):
        for entry in self.conan_data.get("patches", {}).get(self.version, []):
            patch(self, patch_file=os.path.join(self.source_folder, entry["patch_file"]))
