...
template <typename T> struct veh_vec {
    ...
    template <size_t ind> void set_from_index(T val) {
    ...
    template <size_t ind> T get_from_index() {
        static_assert(ind <= 3, "ERROR: index cannot be greater than 3");
        if constexpr (ind == 0) {
            return FL;
        } else if (ind == 1) {
 ...
 */
template <typename T> struct xyz_vec {
    T x;
    T y;
    T z;
};
 ...
 */
template <typename T> struct ypr_vec {
    T yaw;
    T pitch;
    T roll;
};
...
struct DriverInput {
    float requested_accel; // float from 0 to 1 representing percent of accel
                           // pedal travel
    float
        requested_brake; // float from 0 to 1 representing pedal travel of brake
};
...
inline const LidarPoint *points(const foxglove::PointCloud &pc) {
    return reinterpret_cast<const LidarPoint *>(pc.data().data());
}
inline uint64_t num_points(const foxglove::PointCloud &pc) {
    return pc.data().size() / sizeof(LidarPoint);
}
...
struct DriverlessState {
    ...
    const LidarPoint *points() const {
        return lidar_cloud ? core::points(*lidar_cloud) : nullptr;
    }
    uint64_t num_points() const {
        return lidar_cloud ? core::num_points(*lidar_cloud) : 0;
    }
};
...
struct SimulationGroundTruth {
    ...
    double ax_world_mps2{};
    double ay_world_mps2{};
    double yaw_accel_radps2{};
};
...
class StateTracker {
  ...
  public:
    ...
    /**
     * Returns the current driverless state. Consumers judge freshness
     * themselves off the recv times in the struct, since each one wants its own
     * staleness threshold.
     *
    ...
    /**
     * Returns the latest simulation ground truth and whether it is recent
     * enough to use
     */
    std::pair<SimulationGroundTruth, bool> simulation_ground_truth();
  ...
  private:
    ...
    void
    _handle_set_inverter_temps(std::shared_ptr<google::protobuf::Message> msg);
    ...
    SimulationGroundTruth _simulation_ground_truth{};
    std::chrono::steady_clock::time_point _simulation_ground_truth_recv_time{};
    std::mutex _simulation_ground_truth_mutex;
