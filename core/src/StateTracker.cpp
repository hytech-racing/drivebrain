    ...
    std::shared_ptr<google::protobuf::Message> msg)
{
    ...
    else if (msg->GetDescriptor() == hytech_msgs::VNData::descriptor())
    {
        ...
        xyz_vec<float> body_vel_ms = {(in_msg->vn_vel_m_s().x()),
                                      (in_msg->vn_vel_m_s().y()),
                                      (in_msg->vn_vel_m_s().z())};
        ...
        ypr_vec<float> ypr_rad = {(in_msg->vn_ypr_rad().yaw()),
    ...
    else if (msg->GetDescriptor() == foxglove::PointCloud::descriptor())
    {
        // UPDATE driverless vehicle state
             ...
             dv_msgs::SimulationGroundTruth::descriptor())
    {
        auto in_msg =
            std::static_pointer_cast<dv_msgs::SimulationGroundTruth>(msg);

        SimulationGroundTruth truth;

        truth.timestamp_ns = in_msg->timestamp_ns();

        truth.x_world_m = in_msg->x_world_m();
        truth.y_world_m = in_msg->y_world_m();
        truth.yaw_world_rad = in_msg->yaw_world_rad();

        truth.vx_world_mps = in_msg->vx_world_mps();
        truth.vy_world_mps = in_msg->vy_world_mps();
        truth.yaw_rate_radps = in_msg->yaw_rate_radps();

        truth.ax_world_mps2 = in_msg->ax_world_mps2();
        truth.ay_world_mps2 = in_msg->ay_world_mps2();
        truth.yaw_accel_radps2 = in_msg->yaw_accel_radps2();

        {
            std::unique_lock lk(_simulation_ground_truth_mutex);
            _simulation_ground_truth = truth;
            _simulation_ground_truth_recv_time =
...
/* 3 scan periods at 20hz: long enough to ride out a dropped scan, short enough
 * to catch a dead sensor */
...
core::DriverlessState StateTracker::dv_state()
{
    ...
    {
        /* Copying DriverlessState is a shared_ptr refcount bump, not a
         * point-data copy.
         */
        std::unique_lock lk(_dv_state_mutex);
    ...
    if (current_state.lidar_cloud)
    {
        auto scan_time_ns =
            google::protobuf::util::TimeUtil::TimestampToNanoseconds(
                current_state.lidar_cloud->timestamp());
        auto curr_time_ns =
            std::chrono::duration_cast<std::chrono::nanoseconds>(
                std::chrono::system_clock::now().time_since_epoch())
                .count();
        current_state.lidar_is_valid =
            (curr_time_ns - scan_time_ns) <
            std::chrono::nanoseconds(LIDAR_TIMEOUT).count();
    }

    return current_state;
}

void StateTracker::set_dv_path(
...
void StateTracker::set_cone_observations(
    std::shared_ptr<const dv_msgs::Cones> cones)
{
    std::unique_lock lk(_dv_state_mutex);
...
std::pair<core::TeleopCommand, bool> StateTracker::teleop_command()
{
    ...
    bool is_valid = current_command.recv_time.time_since_epoch().count() > 0 &&
                    (std::chrono::steady_clock::now() -
                     current_command.recv_time) < TELEOP_TIMEOUT;

    return {current_command, is_valid};
}

static constexpr std::chrono::milliseconds SIMULATION_GROUND_TRUTH_TIMEOUT{100};

std::pair<core::SimulationGroundTruth, bool>
StateTracker::simulation_ground_truth()
{
    SimulationGroundTruth truth;
    ...
    {
        std::unique_lock lk(_simulation_ground_truth_mutex);
        truth = _simulation_ground_truth;
        recv_time = _simulation_ground_truth_recv_time;
    }

    const bool received = recv_time.time_since_epoch().count() > 0;

    const bool fresh =
        received && (std::chrono::steady_clock::now() - recv_time) <
                        SIMULATION_GROUND_TRUTH_TIMEOUT;
    ...
    const std::array<std::chrono::microseconds, arr_len> &timestamp_arr)
{
    ...
    // TEMP DEBUG
    bool valid =
        within_threshold && all_members_received && last_update_recent_enough;
    if (!valid)
    {
        static int dbg = 0;
        if (dbg++ % 250 == 0)
        {
            spdlog::warn(
                "[DBG] validate_ts: all_recv={} within_thresh={} recent={} "
                "stamps=[{},{},{},{}]",
                all_members_received, within_threshold,
                last_update_recent_enough, debug_copy[0].count(),
                debug_copy[1].count(), debug_copy[2].count(),
                debug_copy[3].count());
        }
    }
    return valid;
